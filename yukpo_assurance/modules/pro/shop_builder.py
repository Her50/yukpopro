...
def _page_panier_checkout_html(boutique: dict, api_base: str = "") -> str:
    ...
    return (
        ...
        f'<div id="cart-similar" class="mb-6"></div>'
        f'<div class="bg-slate-50 rounded-lg p-3 mb-4 flex gap-2">'
        f'<input id="coupon-input" type="text" placeholder="Code promo" '
        f'class="flex-1 px-3 py-2 rounded-lg border border-slate-300 uppercase">'
        f'<button onclick="appliquerCoupon()" class="px-4 py-2 rounded-lg border border-slate-300 font-semibold">Appliquer</button>'
        f'</div><div id="coupon-status" class="text-sm mb-3"></div>'
        ...
        f'let _couponAppliedCode=null;let _couponReduction=0;'
        f'function refresh() {{'
        f'  const c = getCart();'
        ...
        f'  const sub = c.reduce((s,it) => s + it.prix * it.qte, 0);'
        f'  const total = Math.max(0, sub - _couponReduction);'
        f'  document.getElementById("cart-total").innerHTML = (_couponReduction>0?`<span class="text-sm text-slate-500">Sous-total : ${{sub}} {devise}<br>Réduction (${{_couponAppliedCode}}) : -${{Math.round(_couponReduction)}} {devise}<br></span>`:"")+"Total : " + Math.round(total) + " {devise}";'
        ...
        f'  if(c.length) loadSimilar(c);'
        f'  trackAbandonCandidate(c, sub);'
        f'}}'
        f'function retirer(i) {{ const c = getCart(); c.splice(i,1); setCart(c); }}'
        f'window.appliquerCoupon=async function(){{'
        f'  const code=(document.getElementById("coupon-input").value||"").trim().toUpperCase();'
        f'  const st=document.getElementById("coupon-status");'
        f'  if(!code){{ st.textContent=""; _couponAppliedCode=null; _couponReduction=0; refresh(); return; }}'
        f'  const sub=getCart().reduce((s,it)=>s+it.prix*it.qte,0);'
        f'  try{{'
        f'    const r=await fetch("/api/v1/pro/shop/public/{boutique["slug"]}/coupon/valider?code="+encodeURIComponent(code)+"&montant="+sub);'
        f'    const d=await r.json();'
        f'    if(d.ok){{ _couponAppliedCode=code; _couponReduction=d.reduction; st.innerHTML=`<span class="text-emerald-600">✓ ${{d.description||"Réduction appliquée"}}</span>`; refresh(); }}'
        f'    else{{ _couponAppliedCode=null; _couponReduction=0; st.innerHTML=`<span class="text-rose-600">${{d.error||"Code invalide"}}</span>`; refresh(); }}'
        f'  }}catch(_){{ st.textContent="Erreur réseau"; }}'
        f'}};'
        f'async function loadSimilar(c){{'
        f'  const ids=c.map(x=>x.id).filter(Boolean);'
        f'  if(!ids.length) return;'
        f'  try{{'
        f'    const r=await fetch("/api/v1/pro/shop/public/{boutique["slug"]}/cart/similar",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{produit_ids:ids}})}});'
        f'    const d=await r.json();'
        f'    const box=document.getElementById("cart-similar");'
        f'    if(!d.items||!d.items.length){{ box.innerHTML=""; return; }}'
        f'    box.innerHTML=`<h3 class="text-base font-bold mb-2" style="color:var(--primary)">Vous aimerez aussi</h3><div class="grid grid-cols-2 md:grid-cols-3 gap-2">`+d.items.slice(0,6).map(it=>('
        f'      `<a href="${{it.url||"#"}}" class="bg-white rounded-lg border border-slate-200 p-2 hover:shadow-md">${{it.photo_url||it.photo?`<img src="${{it.photo_url||it.photo}}" class="w-full aspect-square object-cover rounded mb-1">`:""}}'
        f'      <div class="text-xs font-semibold truncate">${{it.titre||it.title||""}}</div>'
        f'      <div class="text-xs text-violet-600">${{Math.round(it.prix||it.prix_unit||0)}} ${{it.devise||"{devise}"}}</div></a>`'
        f'    )).join("")+"</div>";'
        f'  }}catch(_){{ }}'
        f'}}'
        f'let _abandonTimer=null;'
        f'function trackAbandonCandidate(c, total){{'
        f'  if(_abandonTimer) clearTimeout(_abandonTimer);'
        f'  if(!c||!c.length) return;'
        f'  _abandonTimer=setTimeout(()=>{{'
        f'    const tel=localStorage.getItem("yk_last_phone");'
        f'    if(!tel) return;'
        f'    fetch("/api/v1/pro/shop/public/{boutique["slug"]}/cart/abandon",{{method:"POST",headers:{{"Content-Type":"application/json"}},body:JSON.stringify({{visitor_telephone:tel,cart_items:c,total}}),keepalive:true}}).catch(()=>{{}});'
        f'  }}, 600000);'  # 10 min d'inactivité
        f'}}'
        f'window.addEventListener("beforeunload",()=>{{'
        f'  const c=getCart();const tel=localStorage.getItem("yk_last_phone");'
        f'  if(c.length&&tel){{'
        f'    const sub=c.reduce((s,it)=>s+it.prix*it.qte,0);'
        f'    navigator.sendBeacon&&navigator.sendBeacon("/api/v1/pro/shop/public/{boutique["slug"]}/cart/abandon",'
        f'      new Blob([JSON.stringify({{visitor_telephone:tel,cart_items:c,total:sub}})], {{type:"application/json"}}));'
        f'  }}'
        f'}});'
        ...
        f'  const tel = fd.get("client_telephone");'
        f'  if(tel) localStorage.setItem("yk_last_phone", tel);'
        f'  const payload = {{ items, client: Object.fromEntries(fd), source: "storefront", code_promo: _couponAppliedCode }};'
