    ...
    <div className={className}>
      ...
      <div ref={wrapRef} className="relative">
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="w-full flex items-center justify-between gap-2 border border-gray-300 rounded-xl px-4 py-2.5 text-sm bg-white hover:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-400"
        >
              ...
              {results.map((c) => (
                <li key={c.code}>
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-amber-50 ${c.code === value ? 'bg-amber-50/50' : ''}`}
                  >
                    <span className="text-lg leading-none">{c.flag}</span>
                    <span className="flex-1 text-left text-gray-800">{c.name}</span>
                    {c.code === value && <Check size={14} className="text-amber-500" />}
                  </button>
                </li>
              ))}
            </ul>
