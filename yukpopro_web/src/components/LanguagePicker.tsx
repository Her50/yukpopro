              ...
              {results.map((l) => (
                <li key={l.code}>
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-amber-50 ${l.code === value ? 'bg-amber-50/50' : ''}`}
                  >
                    <span className="flex-1 text-left text-gray-800">{l.name} <span className="text-gray-400">({l.native})</span></span>
                    {l.code === value && <Check size={14} className="text-amber-500" />}
                  </button>
                </li>
              ))}
            </ul>
