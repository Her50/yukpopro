...
export default function CaissePage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Liste transactions */}
      <div>
        <h2 className="text-sm font-semibold text-gray-700 mb-3">Transactions du jour ({transactions.length})</h2>
        {isLoading ? (
          <div className="flex justify-center py-8"><Loader2 size={22} className="animate-spin text-emerald-600" /></div>
        ) : transactions.length === 0 ? (
          <div className="text-center py-8 text-gray-400 text-sm">Aucune transaction aujourd'hui</div>
        ) : (
          <div className="space-y-2">
            {transactions.map(t => (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end md:items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-gray-900">Enregistrer une transaction</h2>
              ...
              {(['entree', 'sortie'] as const).map(t => (
                <button key={t} onClick={() => setForm(f => ({ ...f, type: t }))}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-semibold capitalize ${
                    form.type === t
                      ? t === 'entree' ? 'bg-green-600 text-white' : 'bg-red-500 text-white'
                      : 'bg-gray-100 text-gray-600'
                  }`}
                >{t === 'entree' ? '➕ Entrée' : '➖ Sortie'}</button>
              ))}
            </div>
